...
const ChatBox = ({ user, newChat, darkMode }) => {
  ...
  useEffect(() => {
    if (user) {
      const conversationId = user.conversionid;
      const conversationMessages = chats[conversationId] || [];
      const updatedMessages = [
        ...newChat.filter((message) => message.sender === "000000" && message.recipient === conversationId),
        ...conversationMessages
      ];
      setMessages(updatedMessages);
      setLoading(false);
    }
  }, [newChat, user, darkMode]);
